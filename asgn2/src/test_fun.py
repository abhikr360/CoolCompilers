...
variable_offsets = {'n' : 0, 'ans' : 4, 'factorial.temp' : 4, 'factorial.n' : 0, 'factorial.res' : 8}


def store(id,val,size=4):
...
# 	def __init__(self, variable_offset = 0,variable_offsets={}, return_addr=None, start_addr=None, prev_start_addr=None, params=[]):
# 		self.variable_offsets = variable_offsets
# 		self.return_addr = return_addr
# 		self.start_addr = start_addr
# 		self.prev_start_addr = prev_start_addr
# 		self.params = params
# 		self.variable_offset = variable_offset
		
# method_stack = [Method(start_addr=0)]
...
def GetReg(stmt, block):
	...
	if 0 in UsableRegisters.values():
		...
		if(l.dataType != 'Array'):
			stmt.code_statement = stmt.code_statement +  "sw $%s, %d($fp)\n"%(VariableData[useless_var][1],variable_offsets[useless_var])
		UsableRegisters[empty_reg] = 0
...
#------------------------------------- Reading Code----------------------------------------
	with open(str(sys.argv[1]), 'rb') as codefile:
		...
		for row in line_reader:
			...
			# print curr_statement.instr_typ
	...
	for x in basic_block_list:
		for st in x:
			...
			if(st.instr_typ == InstrType.ASSIGN and st.operator == None):
				...
				if(st.in1_type == EntryType.INTEGER):
					st.code_statement = st.code_statement + "addiu $sp, $sp, -4\n"
					st.code_statement = st.code_statement + "li $t7, %d\n"%(st.in1)
					st.code_statement = st.code_statement + "sw $t7, ($sp)\n"
				else:
					st.code_statement = st.code_statement + "addiu $sp, $sp, -4\n"
					st.code_statement = st.code_statement + "lw $%s, ($sp)\n"%(VariableData[st.in1][1])
				...
				st.code_statement = st.code_statement + "lw $fp, ($sp)\naddiu $sp, $sp, 4\n"
				
			elif(st.instr_typ == InstrType.FUNC_RETURN):
				if(st.out_type == EntryType.INTEGER):
					st.code_statement = st.code_statement + "li $v0, %d\n"%(st.out)
				else:
					st.code_statement = st.code_statement + "move $v0, $%s\n"%(VariableData[st.out][1])
		...
		for var in VariableData:
			if(VariableData[var][1] <> 0 and LocalSymbolTable[var].dataType <> "Array" ) :
				block_code = block_code + "sw $%s, %d($fp)\n"%(VariableData[var][1], variable_offsets[var])
